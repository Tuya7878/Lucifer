...

{
 ...
 |_   _|                   (_)  .' ..]
   ...
   | |   _[  | | | / /'`\][  |'-| |-'/ /__\\[ `/'`\]
  _| |__/ || \_/ |,| \__.  | |  | |  | \__., | |
 |________|'.__.'_/'.___.'[___][___]  '.__.'[___]
...
{byellow}----> {bcyan}By {bwhite}Vampy Security {bcyan}Github {bwhite}VampySecurity2 {byellow}<-----
...
def update():
    ...
    if (version != git_ver and git_ver != "404: Not Found"):
        ...
        print(logo)
        print(f"{info}Lucifer tiene una actualizacion!\n{info2}Actual: {bred}{version}\n{info}Disponible: {bgreen}{git_ver}\n")
        upask=input(ask+"Quieres actualizar Lucifer (recomendado) pulsa [y] para actualizar o [n] para cancelar --> "+bwhite)
        if upask=="y":
            print(nc)
            system("cd .. && rm -rf Lucifer && git clone https://github.com/vampysecurity2/Lucifer.git && cd WhPhisher && python3 Lucifer.py")
            sprint("\n"+success+"Lucifer  se instalo correctamente :)!! Reinicia la terminal :D!\n")
...
def pexit():
    killer()
    sprint("\n"+info2+bcyan+"Gracias por usar " +bwhite+ "Lucifer! " +bcyan+ "Sigueme en mi Insta, para mas herramientas o dudas :D " +byellow+ "--> " +bwhite+ "vampy security" +bcyan+ " :D\n"+nc)
    exit(0)
...
def about():
    system("clear")
    slowprint(logo)
    print(bcyan+'[Nombre de la herramienta]  '+bpurple+' :[Lucifer] ')
    print(bcyan+'[Version]   '+bpurple+'                 :[2.5]')
    print(bcyan+'[Author]    '+bpurple+'                 :[Vampy Security] ')
    print(bcyan+'[Github]    '+bpurple+'                 :[https://github.com/vampysecurity2] ')
    print(bcyan+'[Instagram] '+bpurple+'                 :[vampy security]  ')
...
def server():
    ...
    slowprint(logo)
    if termux:
        sprint("\n"+info+"Lucifer, la mejor herramienta de phishing :D")
        sleep(1)
